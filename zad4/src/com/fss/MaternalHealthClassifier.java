...
public class MaternalHealthClassifier {

    public static void main(String[] args) {

        int lSize = 200; //number of learning samples for each class

        // 1. Load data
        DataPackage all = new DataPackage();
        all.setFieldSeparator(","); // default ,
        all.setDecimalSeparator('.'); // default .
        all.loadTextFile("Maternal Health Risk Data Set.csv");
        // divide classes
        DataPackage class1 = all.removeVectorsByColumnValue(6, 0);
        DataPackage class2 = all.removeVectorsByColumnValue(6, 1);
        DataPackage class3 = all.removeVectorsByColumnValue(6, 2);
